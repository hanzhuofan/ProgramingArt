...
public class MaxSubArrayLengthTwo {
     ...
     * 1.子数组是连续的，比如[1,3,5,7,9]的子数组有[1,3]，[3,5,7]等等，但是[1,3,7]不是子数组 2.严格上升指在数组上任意位置都满足 nums[i] <
     * nums[i+1]，比如[1,2,2,3]，其中[1,2,2]不是严格上升的子数组，[1,2]是的
     *
     * @param nums
     *            int整型一维数组
     * @return int整型
    ...
    public static int maxSubArrayLengthTwo(int[] nums) {
        ...
        for (int i = 1; i < nums.length; i++) {
            g = Math.max((nums[i] > nums[i - 1]) ? g + 1 : 1, (nums[i] > vh) ? h + 1 : 1);
            h = f + 1;
            f = (nums[i] > nums[i - 1]) ? f + 1 : 1;
            vh = nums[i - 1] + 1;
